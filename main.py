...
def newItemInformation(category_id):
	...
	if login_session['user_id'] != category.user_id:
		...
		if request.method == 'POST':
			newItem = ItemInformation(name=request.form['name'], description=request.form['description'], price=request.form[
							   'price'], make=request.form['make'], category_id=category_id, user_id=category.user_id)
			session.add(newItem)
			session.commit()
			flash('New Catalog %s Item Successfully Created' % (newItem.name))
			return redirect(url_for('showMItem', category_id=category_id))
	else:
		return render_template('newiteminformation.html', category_id=category_id)
...
def editItemInformation(category_id, item_id):
	...
	if request.method == 'POST':
		...
		return render_template('edititeminformation.html', category_id=category_id, menu_id=menu_id, item=editedItem)
