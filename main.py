...
def fbconnect():
    ...
    url = 'https://graph.facebook.com/v2.9/oauth/access_token?grant_type=fb_exchange_token&client_id=%s&client_secret=%s&fb_exchange_token=%s' % (app_id, app_secret, access_token)  # noqa
    h = httplib2.Http()
...
def ItemJSON(category_id, item_id):
    item_information = session.query(ItemInformation).filter_by(id=item_id).one()  # noqa
...
def login_required(f):
    ...
    def decorated_function(*args, **kwargs):
        if 'username' in login_session:
            ...
            flash("You are not allow to access there")
...
def showItem(category_id):
    ...
    if 'username' not in login_session or\
       creator.id != login_session['user_id']:
        return render_template('publiciteminformation.html', items=items,
                               category=category, creator=creator)
    else:
        return render_template('iteminformation.html', items=items,
                               category=category, creator=creator)
...
def newItemInformation(category_id):
    category = session.query(Category).filter_by(id=category_id).one()
    ...
    if request.method == 'POST':
        newItem = ItemInformation(name=request.form['name'],
        ...
        return render_template('newiteminformation.html',
                               category_id=category_id)
...
def editItemInformation(category_id, item_id):
    editedItem = session.query(ItemInformation).filter_by(id=item_id).one()
    category = session.query(Category).filter_by(id=category_id).one()
    if login_session['user_id'] != category.user_id:
        return "<script>function myFunction() {alert('You are not authorized\
        to edit menu items to this category. Please create your own category\
    ...
    if request.method == 'POST':
        ...
        return render_template('edititeminformation.html',
                               category_id=category_id,
...
def deleteItemInformation(category_id, item_id):
    category = session.query(Category).filter_by(id=category_id).one()
    itemToDelete = session.query(ItemInformation).filter_by(id=item_id).one()
    if login_session['user_id'] != category.user_id:
        return "<script>function myFunction()\
